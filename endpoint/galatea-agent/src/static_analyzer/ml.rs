...
impl MlEngine {
    ...
    pub fn predict(&self, features: &Vec<f32>) -> f32 {
        ...
        if let Some(output_val) = outputs
            .get("probabilities")
            .or(outputs.get("output_probability"))
        {
...
pub fn extract_ml_features(pe: &PE, buffer: &[u8]) -> Vec<f32> {
    ...
    let mean_ent = if !entropies.is_empty() {
    ...
    let min_ent = if min_ent == f64::INFINITY {
        0.0
    } else {
        min_ent
    };
    let max_ent = if max_ent == f64::NEG_INFINITY {
        0.0
    } else {
        max_ent
    };
