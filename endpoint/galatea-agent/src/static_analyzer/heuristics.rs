...
use crate::{
    HEUR_ENTROPY_SCORE, HEUR_ENTROPY_THRESHOLD, HEUR_HIDDEN_IMP_SCORE, HEUR_KNOWN_PACKER_SCORE,
    HEUR_RWX_SEC_SCORE, static_analyzer::PackerSignatureEngine, utils::math::calculate_entropy,
};
...
pub fn analyze_pe(
    file_buffer: &[u8],
    file_pe: &PE,
    sig_engine: &PackerSignatureEngine,
) -> Option<HeurReport> {
    ...
    if let Some(packer_name) = sig_engine.scan(&file_pe, &file_buffer) {
    ...
    for section in &file_pe.sections {
        ...
        if start < file_buffer.len() && end <= file_buffer.len() {
            let section_data = &file_buffer[start..end];
    ...
    for import in &file_pe.imports {
        let raw_dll = import.dll.to_lowercase();

        let dll_name = if raw_dll.ends_with(".dll")
            || raw_dll.ends_with(".sys")
            || raw_dll.ends_with(".ocx")
        {
