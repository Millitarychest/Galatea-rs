...
use wdk_sys::{NTSTATUS, STATUS_ACCESS_DENIED, STATUS_INVALID_PARAMETER, STATUS_SUCCESS, UNICODE_STRING};
...
pub(crate) unsafe fn initialize_port(filter: PfltFilter) -> NTSTATUS {
    ...
    let status =
        unsafe { FltBuildDefaultSecurityDescriptor(&raw mut security_descriptor, FLT_PORT_ALL_ACCESS) };
