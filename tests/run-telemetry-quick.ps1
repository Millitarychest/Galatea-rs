...
if (-not $resolvedDbPath -and -not $UseExistingServer) {
    $resolvedDbPath = Join-Path $env:TEMP ("galatea_telemetry_quick_{0}.db" -f ([Guid]::NewGuid().ToString("N")))
}

try {
    if (-not $UseExistingServer) {
        Write-Host "[+] Starting shared quick-test server on $baseUrl"
    ...
    Write-Host "[+] Running quick load checks"
    & "$PSScriptRoot\test-telemetry-load.ps1" -ServerHost $ServerHost -ServerPort $ServerPort -Psk $Psk -UseExistingServer -TotalEvents 1000 -BatchSize 50 -Concurrency 8
    ...
    Write-Host "[OK] Quick telemetry suite passed"
}
finally {
    if ($serverProc -and -not $serverProc.HasExited) {
        Write-Host "[+] Stopping shared quick-test server (PID: $($serverProc.Id))"
